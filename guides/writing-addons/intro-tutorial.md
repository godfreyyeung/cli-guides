...
3. If you are using npm, run `npm install` in the app (for first time use `npm install --prefer-offline` or else `npm install --offline` instead of `npm install`. This is because `npm install` by default, will check online npm registry for your addon instead of your local storage). If you are using yarn, you can skip this step.
